...
The recommended baseline outcome reconciler is a separate observational layer over immutable execution telemetry. Supported launchers invoke a pre-child trusted snapshot or installed copy and perform bounded historical plus current-run reconciliation without changing workload semantics. Per-run locks and atomic sidecars avoid a shared outcome index. See [`run-outcomes.md`](run-outcomes.md) for the evidence and authority boundaries.
...
The separate [`claude-explore`](runtimes/claude-explore.md) runtime launches an existing user-installed Claude Code client for supervised exploration. Its trusted `/bin/sh` bootstrap enters Bash 3.2-compatible privileged-mode policy code with startup variables and exported functions disabled. The runtime validates installed content and Claude's launcher hierarchy, re-resolves updater changes on every invocation, keeps private session controls beneath its stable global-user data root, requires native sandbox failure to be fatal, removes selected credentials, supplies an empty strict MCP configuration, classifies Claude startup arguments, and prepends guarded Git, non-interactive local PostgreSQL, publication, cloud, infrastructure, deployment, container, and remote-capable database commands. Project tools receive the restored developer PATH, while post-launch framework housekeeping retains trusted absolute utility resolution. Its source is under `agent-runtimes/`, not `baseline/` or the adapter taxonomy, because it governs the coding-agent process rather than repository enablement or application execution.
