...
An invocation only considers telemetry records for the current GitHub repository. GitHub.com HTTPS, scp-style SSH, and `ssh://git@github.com/` origins are normalized with the same rules as execution telemetry. Other hosts remain unsupported. A selected run for another repository is rejected. A matching local `path_digest` record is represented as `not_applicable`; it is not queried against GitHub.
...
Reconciliation is trusted host-side and observational. Schema v1 supports GitHub.com repository identities only: every GitHub CLI API request is explicitly bound to `github.com`, and ambient enterprise host/token settings are removed from that subprocess so they cannot redirect evidence collection or receive a GitHub.com token. The reconciler prefers the launcher's freshness-aware token helper, then trusted host `GH_TOKEN`/`GITHUB_TOKEN`, then host GitHub CLI authentication, and finally available unauthenticated public reads. A clear helper-token authentication failure permits the existing single forced-refresh retry; other failures do not. No competing token-minting mechanism exists.

The Codex launcher validates and snapshots its fixed optional adopted sibling before the coding child starts; ambient environment variables cannot select another reconciler. It resolves and validates every external tool used to create and permission outcome-owned temporary resources, copy reconciler code, calculate its digest, launch it after the child, or perform its GitHub/Git reads. Workspace-controlled, temporary, shared-writable, and group/world-writable-ancestry selections are rejected, and only the pinned canonical paths are reused. Protected root- or current-user-owned installation trees remain supported, including Homebrew-style layouts where their ownership and modes satisfy those checks. Snapshot integrity is calculated before and after the child with the pinned Ruby and environment executables and an isolated Ruby load environment, rather than a later PATH lookup. In App mode, the child-facing freshness helper and the trusted outcome helper are separate pre-child files. The trusted helper is never placed in the child environment; both helpers coordinate through the same launcher-owned token state and single renewal worker. Historical and post-run calls use only the trusted snapshot, toolchain, and outcome helper, so child edits cannot cause later execution with launcher-owned GitHub authority.

Claude Explore installs the common reconciler only from within its trusted runtime source tree and never executes a repository's reconciler outside the sandbox. Before the child starts, it searches the original host path for ownership- and mode-safe tools, skips repository-controlled candidates and candidates reached through temporary or writable ancestry, and pins the accepted canonical paths. A rejected early candidate does not prevent selection of a later protected host installation. Captured host GitHub authority, including a host `GH_CONFIG_DIR` where present, is available only to trusted reconciliation. The Claude child receives neither that host config nor the captured token/token-helper path; it receives the runtime's empty private session GitHub config directory instead. These checks avoid deliberate selection from obvious child-controlled storage; they do not claim hostile same-user process containment.

GitHub CLI and Ruby are optional reconciliation prerequisites, not mandatory coding-runtime preflight dependencies. Missing tools, optional snapshot preparation failures, API/authentication/permission/rate-limit/network/timeout/parse failures, and local write failures remain fail-open for the coding workload. Partial outcome setup is removed and emits at most one concise `AGENT_OUTCOME_WARNING:` before the workload continues. A known-unsafe framework reconciler source is still rejected and never executed. Automatic diagnostics never include response bodies or credential values. Useful prior or partial evidence remains intact where it can be written safely.
