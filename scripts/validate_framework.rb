...
class FrameworkValidator
  ...
  RUNTIME_CONFIGURATION_TYPES = %w[codex claude-explore].freeze
  ...
  def validate_supported_runtime(runtime, item_location)
    fields = %w[id display_name status runtime_version distribution description artefacts supported_platforms
                required_executables capabilities limitations configuration]
    ...
    if integer(runtime["runtime_version"], "#{location}.runtime_version") && runtime["runtime_version"] <= 0
    ...
    if runtime["distribution"].is_a?(String) && !RUNTIME_DISTRIBUTIONS.include?(runtime["distribution"])
      error("#{location}.distribution", "unsupported value: #{runtime['distribution'].inspect}; expected one of: #{RUNTIME_DISTRIBUTIONS.join(', ')}")
    end
    validate_runtime_artefacts(runtime["artefacts"], runtime["distribution"], "#{location}.artefacts")
    ...
    string_sequence(runtime["limitations"], "#{location}.limitations", non_empty: false)
  ...
  def validate_runtime_artefacts(value, distribution, location)
    return unless sequence(value, location, non_empty: true)
    ...
    value.each_with_index do |artefact, index|
      ...
      required = %w[role source_path]
      required << "target_path" if distribution == "repository"
      allowed = required.dup
      required.each { |field| error("#{item_location}.#{field}", "missing required field") unless artefact.key?(field) }
      (artefact.keys - allowed).map(&:to_s).sort.each { |field| error("#{item_location}.#{field}", "unknown field") }
      %w[role source_path].each { |field| string(artefact[field], "#{item_location}.#{field}") }
      string(artefact["target_path"], "#{item_location}.target_path") if distribution == "repository"
      if artefact["role"].is_a?(String) && !RUNTIME_ARTEFACT_ROLES.include?(artefact["role"])
        error("#{item_location}.role", "unsupported value: #{artefact['role'].inspect}; expected one of: #{RUNTIME_ARTEFACT_ROLES.join(', ')}")
      end
      roles << [artefact["role"], "#{item_location}.role"]
    end
    validate_unique(roles, label: "artefact role")
  end

  def validate_required_executables(value, location)
    return unless sequence(value, location)

    names = []
    value.each_with_index do |entry, index|
      item_location = "#{location}[#{index}]"
      next unless mapping(entry, item_location)

      error("#{item_location}.name", "missing required field") unless entry.key?("name")
      (entry.keys - %w[name minimum_version]).map(&:to_s).sort.each { |field| error("#{item_location}.#{field}", "unknown field") }
  ...
  def validate_runtime_configuration(value, location)
    ...
    unless RUNTIME_CONFIGURATION_TYPES.include?(type)
      error("#{location}.type", "unsupported configuration type: #{type.inspect}; expected one of: #{RUNTIME_CONFIGURATION_TYPES.join(', ')}") if type.is_a?(String)
      return
    end
    ...
    if type == "codex"
      ...
      if integer(value["policy_schema_version"], "#{location}.policy_schema_version") && value["policy_schema_version"] != 1
        error("#{location}.policy_schema_version", "unsupported policy schema version: #{value['policy_schema_version'].inspect}; expected: 1")
      end
      if string(value["minimum_client_version"], "#{location}.minimum_client_version") && value["minimum_client_version"] != "2.1.224"
        error("#{location}.minimum_client_version", "unsupported minimum client version: #{value['minimum_client_version'].inspect}; expected: 2.1.224")
      end
    end
  end
  ...
  def validate_metadata_paths(metadata)
    ...
    runtimes.each_with_index do |runtime, index|
      ...
      runtime["artefacts"].each_with_index do |artefact, artefact_index|
        next unless artefact.is_a?(Hash)
        location = "framework.yml: #{item_location}.artefacts[#{artefact_index}]"
        validate_source_file(artefact["source_path"], "#{location}.source_path")
        validate_target_path(artefact["target_path"], "#{location}.target_path") if artefact.key?("target_path")
      end
    end
